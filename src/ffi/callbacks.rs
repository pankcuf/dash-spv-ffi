...
pub type ShouldProcessDiffWithRange = unsafe extern "C" fn(base_block_hash: *mut [u8; 32], block_hash: *mut [u8; 32], context: *const c_void) -> u8;
...
pub type MerkleRootLookup = unsafe extern "C" fn(block_hash: *mut [u8; 32], context: *const c_void) -> *mut [u8; 32]; // UIn256
...
pub type GetBlockHashByHeight = unsafe extern "C" fn(block_height: u32, context: *const c_void) -> *mut [u8; 32]; // UIn256
...
pub type HashDestroy = unsafe extern "C" fn(hash: *mut [u8; 32]);
pub type LLMQSnapshotDestroy = unsafe extern "C" fn(snapshot: *mut types::LLMQSnapshot);

fn read_and_destroy_hash<DH>(lookup_result: *mut [u8; 32], destroy_hash: DH) -> Option<UInt256>
    where
        DH: Fn(*mut [u8; 32]) {
    if !lookup_result.is_null() {
        let hash = Some(UInt256(unsafe { *lookup_result }));

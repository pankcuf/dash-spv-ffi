...
impl<'a> TryRead<'a, Endian> for LLMQRotationInfo {
    fn try_read(bytes: &'a [u8], _endian: Endian) -> byte::Result<(Self, usize)> {
        ...
        let mn_list_diff_list_count = bytes.read_with::<dash_spv_primitives::consensus::encode::VarInt>(offset, LE)?.0 as usize;
        let mut mn_list_diff_list_vec: Vec<*mut MNListDiff> = Vec::with_capacity(mn_list_diff_list_count as usize);
